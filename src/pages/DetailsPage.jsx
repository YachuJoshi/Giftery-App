        ...
        <div className="row">
          ...
          <div className={styles.container}>
            <div className={styles.imageContainer}>
              <img src={product.imageURL} alt="Gift" className={styles.productImage} />
            </div>
            <div className={styles.detailsContainer}>
              <h4 className={styles.productLabel}>
                {product.label.map((label, index) => (
                  index !== product.label.length - 1 ? `${label} / ` : `${label}`
                ))}
              </h4>
              <h2 className={styles.productTitle}>{product.name}</h2>
              <p className={styles.productDescription}>{product.description}</p>
              <p className={styles.productPrice}>
                NPR {quantity === '' ? Math.round(product.price) : Math.round(quantity * product.price)}
              </p>
              ...
              <p className={styles.socialSites}>
                Liked It? Share it on:
                <ul className={styles.socialList}>
                  <li><FaFacebookF style={{ color: "#3b5999" }} className={styles.socialIcon} /> </li>
                  <li><FaInstagram style={{ color: "#e4405f" }} className={styles.socialIcon} /> </li>
                  <li><FaPinterestP style={{ color: "#bd081c" }} className={styles.socialIcon} /></li>
                  <li><FaTwitter style={{ color: "#55acee" }} className={styles.socialIcon} /> </li>
                </ul>
              </p>
            </div>
          </div>
